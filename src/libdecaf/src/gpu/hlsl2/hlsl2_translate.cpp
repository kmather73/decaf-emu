   ...
   for (auto i = 0u; i < numInstSrc; ++i) {
      ...
      if (src.type == SrcVarRef::Type::CBUFFER) {
         ...
         if (cbuffer.indexMode == CbufferIndexMode::None) {
            ...
            mCbufferNumUsed[cbuffer.bufferId] = latte::MaxUniformBlockSize;
         }
...
bool Transpiler::translate(gpu::ShaderDesc *shaderDesc, Shader *shader)
{
   ...
   auto funcVars = []() {
      ...
      vout << "  float4 PV, PVo;\n";
      vout << "  float PS, PSo;\n";
      vout << "  bool activeMask;\n";
      vout << "  bool activePredicate;\n";
      vout << "  float4 tmp;\n";
      vout << "  uint4 utmp;\n";
      vout << "\n";
      return vout.str();
   ...
   if (shaderDesc->type == gpu::ShaderType::Vertex) {
      ...
      vsShader->samplerUsed = state.mSamplerUsed;
      vsShader->textureUsed = state.mTextureUsed;
      ...
      // Not quite sure what this is either
      decaf_check(state.mPsInControl0.BARYC_SAMPLE_CNTL() == 1);

      // We do not currently support parameter generation
      decaf_check(!state.mPsInControl0.PARAM_GEN());
      //state.mPsInControl0.PARAM_GEN_ADDR();

      if (state.mPsInControl0.LINEAR_GRADIENT_ENA()) {
         decaf_abort("Unexpected linear gradient mode...");
      } else {
         // PERSP_GRADIENT_ENA is always true here...
      }
      ...
      psShader->samplerUsed = state.mSamplerUsed;
      psShader->textureUsed = state.mTextureUsed;
   } else {
