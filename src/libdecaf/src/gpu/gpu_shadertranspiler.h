...
protected:
   ...
   {
      translateCfAluGeneric(cf, AluPredMode::Active);
   }
   ...
   GprRef findVtxSemanticGpr(uint32_t semanticId) const
   {
      ...
      // If this ever happens, what it really means is just that we
      //  need to abort this fetch, because VS doesn't use it!
      decaf_abort("Failed to find valid semantic output GPR");
   }
