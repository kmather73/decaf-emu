...
public:
      ...
      readOneGroup()
   {
      ...
      if (literalCount > 0) {
         ...
         group.literals = gsl::as_span(reinterpret_cast<const uint32_t*>(&literalSpan[0]), literalCount);
...
inline SrcVarRef
makeSrcVar(const ControlFlowInst &cf, const AluInstructionGroup &group, const AluInst &inst, uint32_t srcIndex)
{
   auto instFlags = getInstructionFlags(inst);
   auto valueType = VarRefType::FLOAT;
