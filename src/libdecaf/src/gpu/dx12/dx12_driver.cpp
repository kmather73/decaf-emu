...
void Driver::executeBuffer(pm4::Buffer *buffer)
{
   ...
   // Free the heaps if there arent enough items to feel safe
   if (mRtvHeap->maxItems - mRtvHeap->usedItems < 100) {
      mFrameData->rtvHeap = mRtvHeap;
      mRtvHeap = nullptr;
   }

   if (mSamplerHeap->maxItems - mSamplerHeap->usedItems < 1000) {
      mFrameData->samplerHeap = mSamplerHeap;
      mSamplerHeap = nullptr;
   }

   if (mSrvHeap->maxItems - mSrvHeap->usedItems < 1000) {
