...
void
DecafSDLDX12::drawScanBuffer(float vp[4], ID3D12Resource *texture, uint32_t bufferIdx)
{
   ...
   viewport.TopLeftX = vp[0];
   viewport.TopLeftY = vp[1];
   viewport.Width = vp[2];
...
void
DecafSDLDX12::drawScanBuffers(float tv[4], ID3D12Resource *tvBuffer, float drc[4], ID3D12Resource *drcBuffer)
{
   ...
   if (tvBuffer || drcBuffer) {
      ...
      float adjTv[4];
      adjTv[0] = tv[0];
      adjTv[1] = height - (tv[1] + tv[3]);
      adjTv[2] = tv[2];
      adjTv[3] = tv[3];
      float adjDrc[4];
      adjDrc[0] = drc[0];
      adjDrc[1] = height - (drc[1] + drc[3]);
      adjDrc[2] = drc[2];
      adjDrc[3] = drc[3];

      if (tvBuffer && adjTv[2] && adjTv[3]) {
         drawScanBuffer(adjTv, tvBuffer, 0);
      }

      if (drcBuffer && adjDrc[2] && adjDrc[3]) {
